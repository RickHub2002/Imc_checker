function start() {
    ...
    inputHeight.addEventListener("input", function () {
        let value = this.value.replace(/[^0-9.]/g, ""); // Permite números e o ponto

        // Garantir que o valor comece com "1"
        if (value.charAt(0) !== "1") {
            value = "1" + value;
        }

        // Coloca o ponto após o primeiro número se houver mais de 1 dígito
        if (value.length > 2) {
            // Coloca o ponto logo após o primeiro dígito
            value = value.slice(0, 2) + "." + value.slice(2);
        }

        // Garante que o valor não termine com ponto
        if (value.charAt(value.length - 1) === '.') {
            value = value.slice(0, -1); // Remove o ponto final
        }
